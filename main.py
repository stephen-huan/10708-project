...
for n_iters in iterations:
    ...
    print(
        f"Jacobi took {toc - tic} seconds to run {n_iters} iterations with relative L2 loss {jacobi_accuracies[-1]}"
    )
...
for n_iters in iterations:
    ...
    print(
        f"Gauss Seidel took {toc - tic} seconds to run {n_iters} iterations with relative L2 loss {gauss_seidel_accuracies[-1]}"
    )
