...
def initialize_solution(n, boundaries=[0, 0, 0, 0]):
    ...
    solution = mean_value * np.ones((n+2,n+2)) # n+2 to include the boundary values
    ...
    for _ in range(n_iters):
        ...
        for i in range(1, n-1):
            for j in range(1, n-1):
                solution_new[i,j] = 0.25 * (solution[i-1,j] + solution[i+1,j] + solution[i,j-1] + solution[i,j+1])
